...
if [[ ! -f "$PIPELINE_UNIT" ]]; then
  needs_install=1
elif grep -q "%h/xAIO-URL-Agent" "$PIPELINE_UNIT"; then
  # Unit still contains the template path; reinstall to bake in actual repo path.
  if [[ "$PROJECT_DIR" != "$DEFAULT_REPO" ]]; then
    needs_install=1
  fi
elif ! grep -q "$PROJECT_DIR" "$PIPELINE_UNIT"; then
  # Unit points somewhere else; reinstall.
  needs_install=1
fi
